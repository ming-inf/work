...
public class LightningSimulator extends Application {
  ...
  public void start(Stage stage) throws Exception {
    ...
    field.addEventFilter(LightningEvent.PLASMA_STRIKE, event -> System.out.println("Field filtered strike: " + event.getI() + ", " + event.getJ()));

    field.addEventHandler(LightningEvent.PLASMA_STRIKE, event -> System.out.println("Field handled strike: " + event.getI() + ", " + event.getJ()));

    periodicallyStrikeRandomNodes(field);
  }

  private void periodicallyStrikeRandomNodes(TilePane field) {
    Timeline timeline = new Timeline(new KeyFrame(Duration.seconds(0), event -> strikeRandomNode(field)), new KeyFrame(Duration.seconds(2)));
  ...
  private void strikeRandomNode(TilePane field) {
    LightningReactor struckNode = (LightningReactor) field.getChildren().get(random.nextInt(FIELD_SIZE * FIELD_SIZE));
  ...
  static class LightningEvent extends Event {
    ...
    /**
     * Creates a new {@code LightningEvent} with an event type of {@code PLASMA_STRIKE}. The source and target of the event is set to
    ...
    /**
     * Construct a new {@code LightningEvent} with the specified event source and target. If the source or target is set to {@code null}, it is replaced by
     * the {@code NULL_SOURCE_TARGET} value. All LightningEvents have their type set to {@code PLASMA_STRIKE}.
     *
